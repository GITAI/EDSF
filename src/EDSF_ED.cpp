...
ED::ED(Mat _srcImage, GradientOperator _op, int _gradThresh, int _anchorThresh, int _scanInterval, int _minPathLen, int _kSize, double _sigma, bool _sumFlag)
{
	...
	segmentNos = 0;
	segmentPoints.emplace_back(vector<Point>()); // create empty vector of points for segments
	...
	if (selectStableAnchors) {
		...
		for (int i = 0; i < width * height; i++)
			if (edgeImg[i] == ANCHOR_PIXEL)
				...
				int y = i / width;
				int x = i % width;
				anchorPoints.emplace_back(Point(x, y)); // push validated anchor point to vector
			}
		...
		for (int j = start; j < width - 2; j += inc) {
			...
			if (dirImg[i * width + j] == EDGE_VERTICAL) {
				...
				if (diff1 >= anchorThresh && diff2 >= anchorThresh) {
					...
					anchorPoints.emplace_back(Point(j, i));
				}
			...
			if (totalLen > 0) {
				...
				for (int k = count - 1; k >= 0; k--) {
					...
					for (int l = chains[chainNo].len - 1; l >= 0; l--) {
						segmentPoints[segmentNos].emplace_back(chains[chainNo].pixels[l]);
						noSegmentPixels++;
					} //end-for
